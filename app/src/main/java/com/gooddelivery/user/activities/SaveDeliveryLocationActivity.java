...
public class SaveDeliveryLocationActivity extends FragmentActivity implements OnMapReadyCallback,
    ...
    com.gooddelivery.user.models.Address address = null;
    ...
    Retrofit retrofit;
    int PLACE_AUTOCOMPLETE_REQUEST_CODE = 1003;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        address = new com.gooddelivery.user.models.Address();
        customDialog = new CustomDialog(context);
        address.setType(getString(R.string.other));
...
// Construct a request object, passing the place ID and fields array.
                FetchPlaceRequest request = FetchPlaceRequest.builder(id, fields).build();
               PlacesClient placesClient = com.google.android.libraries.places.api.Places.createClient(this);
                placesClient.fetchPlace(request).addOnSuccessListener((response) -> {
                    ...
                    public void onCanceled() {
                        String ex="cancled";
                        Log.e(TAG, "Place not found: " + "Cancelled");
                    }
                });
            }
            ...
            if (isEdit != null && isEdit.equals("yes")) {
                if (GlobalData.selectedAddress != null) {
                    ...
                    txtAddress.setText(address.getMapAddress());
...
//            getAddress( location.getLatitude(), location.getLongitude());
    ...
    public void getAddress(final Context context, final double latitude, final double longitude) {
        ...
        Call<ResponseBody> call = apiInterface.getResponse(latitude + "," + longitude,
                context.getResources().getString(R.string.map_key));
...
//            getAddress(context, srcLat, srcLng);
    ...
    public void onCameraMove() {
        ...
        txtAddress.setText(this.getResources().getString(R.string.getting_address));
        animationLineCartAdd.setVisibility(View.VISIBLE);
    ...
    private void saveAddress(String update) {
        if (address != null && address.getMapAddress() != null && validate()) {
            ...
            Call<com.gooddelivery.user.models.Address> call = apiInterface.saveAddress(address,update);
            call.enqueue(new Callback<com.gooddelivery.user.models.Address>() {
                @Override
                public void onResponse(@NonNull Call<com.gooddelivery.user.models.Address> call, @NonNull Response<com.gooddelivery.user.models.Address> response) {
                ...
                @Override
                public void onFailure(@NonNull Call<com.gooddelivery.user.models.Address> call, @NonNull Throwable t) {
    ...
    private void showUpdateAddressAlert() {
        ...
        builder.setTitle(getString(R.string.update_address));
        builder.setMessage(getString(R.string.are_you_want_update_address));
    ...
    private void updateAddress() {
        if (address.getType().equalsIgnoreCase(getString(R.string.other))) {
            address.setType(otherAddressHeaderEt.getText().toString());
        }
        if (address != null && address.getId() != null && validate()) {
            customDialog.show();
            apiInterface = ApiClient.getRetrofit().create(ApiInterface.class);
            Call<com.gooddelivery.user.models.Address> call = apiInterface.updateAddress(address.getId(), address);
            call.enqueue(new Callback<com.gooddelivery.user.models.Address>() {
                @Override
                public void onResponse(@NonNull Call<com.gooddelivery.user.models.Address> call, @NonNull Response<com.gooddelivery.user.models.Address> response) {
                    customDialog.dismiss();
    ...
    private boolean validate() {
        if (address.getMapAddress().isEmpty() && address.getMapAddress().equals(getResources().getString(R.string.getting_address))) {
            ...
            Toast.makeText(this, getString(R.string.please_enter_house_flat), Toast.LENGTH_SHORT).show();
            return false;
        } else if (address.getLandmark().isEmpty()) {
            Toast.makeText(this, getString(R.string.please_enter_landmark), Toast.LENGTH_SHORT).show();
            return false;
        } else if (address.getLatitude() == null || address.getLongitude() == null) {
            Toast.makeText(this, getString(R.string.lat_lng_cannot_be_empty), Toast.LENGTH_SHORT).show();
            return false;
        } else {
            return true;
        }
    ...
    @OnClick({R.id.backArrow, R.id.save, R.id.imgCurrentLoc, R.id.cancel_txt, R.id.skip_txt, R.id.address_lay})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.save:
                ...
                address.setMapAddress(txtAddress.getText().toString());
                address.setBuilding(flatNoEdit.getText().toString());
                address.setLandmark(landmark.getText().toString());
                if (address.getType().equalsIgnoreCase(getString(R.string.other)) || address.getType().equalsIgnoreCase("")) {
                    address.setType(otherAddressHeaderEt.getText().toString());
                }
                if (address.getBuilding().equalsIgnoreCase("")) {
                    Toast.makeText(context, getString(R.string.please_enter_house_flat), Toast.LENGTH_SHORT).show();
                } else if (address.getLandmark().equalsIgnoreCase("")) {
                    Toast.makeText(context, getString(R.string.please_enter_landmark), Toast.LENGTH_SHORT).show();
                } else {
