...
import com.gooddelivery.user.adapter.OrderFlowAdapter;
...
import com.gooddelivery.user.fragments.OrderViewFragment;
import com.gooddelivery.user.helper.CustomDialog;
import com.gooddelivery.user.helper.DataParser;
...
import com.gooddelivery.user.models.OrderFlow;
...
import static com.gooddelivery.user.helper.GlobalData.ORDER_STATUS;
...
//            updateOrderDeatail();
            ...
            orderIdTxt.setText(getString(R.string.order) + " #000" + order.getId().toString());
    ...
    private String getUrl(double source_latitude, double source_longitude, double dest_latitude, double dest_longitude) {
        ...
        String parameters = str_origin + "&" + str_dest + "&" + sensor+"&"+"key="+ BuildConfigure.DIRECTION_KEY;
...
//                    if (isSelectedOrder.getInvoice().getPaymentMode().equalsIgnoreCase("tbl") && isSelectedOrder.getStatus().equalsIgnoreCase("ARRIVED")) {
//                        Integer id = isSelectedOrder.getId();
//                        String url = BuildConfigure.BASE_URL + "transbank_payment?order=" + id;
